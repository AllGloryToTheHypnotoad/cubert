...
# External modules -----------------------
from scapy.all import sniff, DNS, IP, UDP, ARP, Ether, srp, arping, conf
...
LAN = "10.0.1."
ARP_RESCAN_TIME = 60
# timer = Timer(ARP_RESCAN_TIME, arp_scan)
...
def arp_scan():
    ...
    ans, _ = arping(LAN + "0/24", verbose=0)
    with data_lock:
        for _, received in ans:
            ip = received.psrc
            mac = received.hwsrc
            # vendor = conf.manufDB.get(mac, "Unknown")
            name, method = get_hostname(ip)
            vendor = conf.manufdb.lookup(mac)[1]
            host_data[mac]
            host_data[mac]["ip"] = ip
...
def handle_dns_pkt(packet):
    ...
    if packet[DNS].qr != 1:  # DNS response
        debug(f"ERROR: invalid DNS packet: {packet[DNS]}", Fore.RED)
        return
...
def save_to_yaml():
    ...
    try:
        ...
        for mac, data in host_data.items():
            # print(type(data["vender"]),data["vender"])
            yaml_data[mac] = {
                'ip': data['ip'],
...
def listen_main():
    ...
    try:
        while run:
            ...
            sniff(prn=packet_callback, filter=sniff_filter, timeout=1.0, store=False)
    # except Exception as e:
...
def main():
    ...
    #     # print(host_data)
        ...
        time.sleep(1)
        # print("finally")
        listen_t.join(timeout=5.0)
        print_summary()
        print(f"{Fore.GREEN}/// main() finished ///{Fore.RESET}")
        save_to_yaml()
        print(f"{Fore.GREEN}/// Saving host_data to {YAML_FILE} ///{Fore.RESET}")
